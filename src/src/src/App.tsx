...
import { Search, BookOpen, Code2, TerminalSquare, Play, ChevronRight, Rocket, PlugZap, Cpu, Shield, GitBranch, Server, Smartphone, Database, Gauge, Copy, Check, Globe, Network } from "lucide-react";
...
async function jfetch(path: string, body?: any) {
  ...
  const opts: RequestInit = body ? { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) } : { method: 'GET' };
...
function useClipboard(text: string, timeout = 1500) {
  ...
  return { copied, async copy(){ await navigator.clipboard.writeText(text); setCopied(true); setTimeout(()=>setCopied(false), timeout); } };
}
...
export default function DevPortal() {
  ...
  function EndpointForm(){
    ...
    const path = meta.path(); const body = meta.body ? meta.body() : undefined;
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="grid md:grid-cols-3 gap-3">
          {endpoint==='getBlockHeader' && (
            <label className="text-sm">height
              <input type="number" value={params.height ?? ''} onChange={e=>setParams(p=>({ ...p, height:Number(e.target.value) }))} className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50" placeholder="100"/>
            </label>
          )}
          {(endpoint==='getBalance' || endpoint==='getUTXO') && (
            <label className="text-sm">address
              <input value={params.address || ''} onChange={e=>setParams(p=>({ ...p, address:e.target.value }))} className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50" placeholder="nak1q..."/>
            </label>
          )}
          {endpoint==='sendRawTransaction' && (
            <label className="text-sm col-span-3">rawTx hex
              <textarea value={params.hex || ''} onChange={e=>setParams(p=>({ ...p, hex:e.target.value }))} className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50 h-28" placeholder="02000000..."/>
            </label>
          )}
        </div>

        <div className="flex gap-2 items-center">
          <button onClick={run} className="px-4 py-2 rounded-lg bg-black text-white flex items-center gap-2 disabled:opacity-50" disabled={loading}>
        ...
        <div>
          ...
          <div className="rounded-lg border bg-white/50 p-3 overflow-auto max-h-80">
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white text-slate-900">
      <header className="sticky top-0 z-30 backdrop-blur bg-white/70 border-b">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          <Cpu className="w-5 h-5"/><h1 className="font-bold">Nakamoto Developer Portal</h1>
          <div className="ml-auto relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-500"/>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search docs…" className="pl-9 pr-3 py-2 rounded-lg border bg-white/60"/>
          </div>
        </div>
      </header>

      <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-[260px_1fr] gap-6 py-6">
        <aside className="space-y-2">
          {nav.map(item => (
            <button key={item.key} onClick={()=>setActive(item.key)} className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl border ${active===item.key?'bg-black text-white border-black':'bg-white/60 hover:bg-white'}`}>
              {item.icon}<span>{item.label}</span><ChevronRight className="w-4 h-4 ml-auto opacity-60"/>
            </button>
          ))}
          <div className="mt-6 p-3 rounded-xl border bg-white/60">
            <div className="text-xs font-semibold mb-2">Quick Links</div>
            <ul className="text-sm space-y-1 list-disc pl-5">
              <li>RPC: <code className="rounded bg-slate-100 px-1">{nodeUrl}</code></li>
      ...
      <footer className="border-t py-6 text-center text-sm text-gray-500">Built with ❤️ for the Nakamoto community — {new Date().getFullYear()}</footer>
    </div>
  );
}
...
function Card({ title, icon, children }:{ title:string, icon?:React.ReactNode, children:React.ReactNode }){
  return (<div className="rounded-2xl border bg-white/70 p-5 shadow-sm">
    <div className="flex items-center gap-2 mb-3">{icon}<h3 className="font-semibold text-lg">{title}</h3></div>{children}
  </div>);
}
...
function CodeBlock({ title, code }:{ title:string, code:string }){
  ...
  return (<div className="relative">
    ...
    <div className="rounded-xl overflow-hidden border">
      ...
      <SyntaxHighlighter language="bash" style={oneDark} customStyle={{ margin: 0, borderRadius: 0 }}>{code}</SyntaxHighlighter>
    </div>
  </div>);
}
    ...
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-2 gap-4">
        <Card title="What is Nakamoto?" icon={<Shield className="w-4 h-4"/>}>
          <p className="text-sm leading-6">Nakamoto is a mobile-native, zk-verified Layer 1 with DPoS + HotStuff finality and a Proof-of-Verification layer run by smartphones. The base layer focuses on payments & staking, with a fixed 21M supply and 2-year halvings.</p>
        </Card>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card title="Consensus" icon={<GitBranch className="w-4 h-4"/>}><p className="text-sm">HotStuff pipeline: propose → prevote → precommit → commit. Finality when QC ≥ 2/3 and PoV ≥ 67%.</p></Card>
        <Card title="Mobile Verifier" icon={<Smartphone className="w-4 h-4"/>}><p className="text-sm">Phones verify zk-proof in ~80ms and sample k=50 chunks (DAS). Attestations are rewarded.</p></Card>
        <Card title="Networking" icon={<Network className="w-4 h-4"/>}><p className="text-sm">libp2p over QUIC, NAT traversal via relays, topics for blocks/tx/att/chunks.</p></Card>
      </div>
    </div>
  );
}
...
5) POST /submitAttestation`;
  return (
    <div className="space-y-6">
      <Card title="Payments Tutorial" icon={<span className="inline-flex w-4 h-4 items-center justify-center rounded bg-black text-white text-[9px]">W</span>}><SyntaxHighlighter language="markdown" style={oneDark}>{t1}</SyntaxHighlighter></Card>
      <Card title="PoV Attester Tutorial" icon={<Shield className="w-4 h-4"/>}><SyntaxHighlighter language="markdown" style={oneDark}>{t2}</SyntaxHighlighter></Card>
    </div>
  );
}
...
function StatusCard(){
  ...
  useEffect(()=>{ (async () => {
    ...
    try { setSync(await jfetch('/getSyncStatus')); } catch(_){ /* ignore */ }
  })(); }, []);
  return (
    <div className="space-y-4">
