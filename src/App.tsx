...
import {
  Search, BookOpen, Code2, TerminalSquare, Play, ChevronRight, Rocket,
        ...
        <div className="grid md:grid-cols-2 gap-3">
          <label className="text-sm">Node URL
            <input value={nodeUrl} onChange={e=>setNodeUrl(e.target.value)}
                   className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50"
        ...
        <div className="grid md:grid-cols-3 gap-3">
          {endpoint==='getBlockHeader' && (
            <label className="text-sm">height
              <input type="number" value={params.height ?? ''} onChange={e=>setParams(p=>({ ...p, height:Number(e.target.value) }))}
                     className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50" placeholder="100"/>
            </label>
          )}
          {(endpoint==='getBalance' || endpoint==='getUTXO') && (
            <label className="text-sm">address
              <input value={params.address || ''} onChange={e=>setParams(p=>({ ...p, address:e.target.value }))}
                     className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50" placeholder="nak1q..."/>
            </label>
          )}
          {endpoint==='sendRawTransaction' && (
            <label className="text-sm col-span-3">rawTx hex
              <textarea value={params.hex || ''} onChange={e=>setParams(p=>({ ...p, hex:e.target.value }))}
                        className="w-full mt-1 px-3 py-2 rounded-md border bg-white/50 h-28" placeholder="02000000..."/>
            </label>
          )}
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <div className="ml-auto relative">
            ...
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search docs…"
                   className="pl-9 pr-3 py-2 rounded-lg border bg-white/60"/>
          </div>
        </div>
      </header>
        ...
        <aside className="space-y-2">
          {nav.map(item => (
            <button key={item.key} onClick={()=>setActive(item.key)}
                    className={`w-full flex items-center gap-2 px-3 py-2 rounded-xl border ${active===item.key?'bg-black text-white border-black':'bg-white/60 hover:bg-white'}`}>
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="grid md:grid-cols-2 gap-4">
        <Card title="What is Nakamoto?" icon={<Shield className="w-4 h-4"/>}>
          <p className="text-sm leading-6">
            Nakamoto is a mobile-native, zk-verified Layer 1 with DPoS + HotStuff finality and a Proof-of-Verification layer run by smartphones.
            The base layer focuses on payments & staking, with a fixed 21M supply and 2-year halvings.
          </p>
        </Card>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <Card title="Consensus" icon={<GitBranch className="w-4 h-4"/>}>
          <p className="text-sm">HotStuff pipeline: propose → prevote → precommit → commit. Finality when QC ≥ 2/3 and PoV ≥ 67%.</p>
        </Card>
        <Card title="Mobile Verifier" icon={<Smartphone className="w-4 h-4"/>}>
          <p className="text-sm">Phones verify zk-proof in ~80ms and sample k=50 chunks (DAS). Attestations are rewarded.</p>
        </Card>
        <Card title="Networking" icon={<Network className="w-4 h-4"/>}>
          <p className="text-sm">libp2p over QUIC, NAT traversal via relays, topics for blocks/tx/att/chunks.</p>
        </Card>
      </div>
    </div>
  );
